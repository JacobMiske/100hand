# 100 hand poker game and simulator
# Jacob Miske
# MIT License
...
def play_hand_hold_highest_card(player_hand):
    ...
    highest_value = 0
    highest_value_card_index = 0
    for i, count in enumerate(player_hand, 0):
        i_value = card_order_dict[i[0]]
        if i_value > highest_value:
            highest_value = i_value
            highest_value_card_index = count
    return [highest_value_card_index]
...
def play_hand_hold_highest_value_combination(players_hand):
    ...
    # If only highest card, will return result of play_hand_hold_highest_card()
    highest_value_combination = score_hand(player_hand=players_hand)
...
def simulate_gameplay():
    """
    Static function to run simulated 100 hand gameplay
    TODO: Replace all 'input' functions within user_gameplay to simulation chosen choices
    :return:
    """
    print("Welcome to 100 hand poker simulator")
    #credit = input("how many credits do you want the AI to start with? (must be an int > 100)")
    #credit = int(credit)
    credit = 2000
    ...
    while credit > 0:
        ...
        deck = terminal_playing_cards.Deck()
        deck.shuffle()
        simulated_hand = terminal_playing_cards.View([deck.pop() for _ in range(5)])
        simulated_cards = simulated_hand.cards

        credit_from_round = 0
        bet = 1 # Assume 1 cent per hand now (independent variable so it only should be a variable on speed of loss
        # input("What is your per hand bet? (1 to 5 cents inclusive)")
        number_of_hands = 20 # TODO: hardcoded now, need to change later
        # input("How many hands are played at once? (1 to 100 inclusive)")
        input_cost = int(bet) * int(number_of_hands)
        print(simulated_hand)
        simulated_simple_hand = get_simple_hand_data_structure(simulated_hand)
        print(simulated_simple_hand)
        quit()
        held_cards = input(
            "Cards 1 thru 5 are shown, list which cards should be held with commas between each (e.g. 1, 3, 4)")
        held_cards_index = [int(i) for i in held_cards.split(",")]
        ...
        cards_to_return = []
        for i in held_cards_index:
            cards_to_hold.append(simulated_cards[i - 1])
        for i in return_cards_index:
            cards_to_return.append(simulated_cards[i - 1])
        print(cards_to_return)
        for card in cards_to_return:
            # insert cards to return (not held) back into the deck at zero index, one at a time
            # TODO: test case should verify that deck.cards length is equal to 52-len(cards_to_hold)
        ...
        # Add new cards to saved cards to make new hand
        simulated_hand.cards = cards_to_hold
        print(simulated_hand)
        held_cards_hand = simulated_hand
        # deal in new cards after another shuffle
        deck.shuffle()
        new_cards = terminal_playing_cards.View([deck[i] for i in range(cards_to_add)])
        print(new_cards)
        for card in new_cards.cards:
            simulated_hand.cards.append(card)
        simulated_hand._max = 5
        # This is the hand to score!
        print(simulated_hand)
        # Convert complex form of cards to simple form for quick processing
        shorthand_player_hand = get_simple_hand_data_structure(terminal_cards_hand=simulated_hand)
        # print(shorthand_player_hand)
        result = score_hand(player_hand=shorthand_player_hand)
        print(result)
        # Score points for the first hand pulled before scoring the other 99 or so
